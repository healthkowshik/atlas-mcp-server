...
- [x] T001 Create `pyproject.toml` at repository root with runtime dep `fastmcp>=3.0`, dev deps `pytest`, `pytest-asyncio`, `geonamescache>=2.0`, `pycountry>=24.6`, `ruff`, `mypy`, and project metadata (name: `atlas-mcp-server`, version: `0.1.0`, requires-python: `>=3.12`)
- [x] T002 Create `.python-version` at repository root containing `3.12`
- [x] T003 [P] Create `src/atlas/__init__.py` (empty), `src/atlas/data/` directory, and `src/atlas/data/.gitkeep` so the data directory is tracked before geo_corpus.json is generated
- [x] T004 [P] Create `tests/__init__.py`, `tests/contract/__init__.py`, `tests/integration/__init__.py`, `tests/unit/__init__.py` (all empty), and `tests/conftest.py` with a placeholder comment — to be populated in Phase 2
- [x] T005 [P] Create `scripts/__init__.py` (empty) so `scripts/` is a recognisable Python directory; confirm `uv sync --all-extras` resolves without errors
...
- [x] T006 Write `scripts/build_corpus.py`: use `pycountry` (countries + ISO 3166-2 subdivisions) and `geonamescache` (cities with population ≥ 500 000) to collect geographic names; normalise each name with `unicodedata.normalize("NFKD", ...)` stripping combining characters; lowercase; deduplicate; include 7 hardcoded continents; write sorted list to `src/atlas/data/geo_corpus.json` as UTF-8 JSON; print final count to stdout
- [x] T007 Run `uv run python scripts/build_corpus.py` and verify `src/atlas/data/geo_corpus.json` is created, contains ≥ 5 000 entries, and all entries are lowercase strings
...
- [x] T013 [P] [US1] Write FAILING contract test in `tests/contract/test_tool_schemas.py`: list all tools from the server; find `atlas_start`; assert `inputSchema["properties"] == {}` (no parameters); assert `atlas_start` is present in tool names
...
- [x] T017 [P] [US2] Write FAILING contract test in `tests/contract/test_tool_schemas.py` for `atlas_play`: assert tool is registered; assert `inputSchema["properties"]` contains `"word"` with type `"string"`; assert `"word"` is in `inputSchema.get("required", [])`
...
- [x] T024 [P] [US4] Write FAILING contract test in `tests/contract/test_tool_schemas.py` for `atlas_concede`: assert tool is registered; assert `inputSchema["properties"] == {}` (no parameters)
...
- [x] T026 [US4] Implement `render_game_over(winner: str, words_played: list[str], first_mover: str) -> str` in `src/atlas/display.py`: display winner announcement (🏆 for winner, 😔 for loser); display numbered word chain with player attribution (odd positions = first_mover, even positions = other player); append "Type `atlas_start` to play again!" — this function is called by both `atlas_concede` (user loses) and the server-concession path in `atlas_play` (user wins)
- [x] T027 [US4] Add `atlas_concede` async tool to `src/atlas/server.py`: `ctx: Context`; load session — if None or `game_active=False`, return `"No game in progress. Call atlas_start to begin."`; set `game_active=False`, `winner="server"`, save session; return `render_game_over(winner="server", words_played=session["words_played"], first_mover=session["first_mover"])`
- [x] T028 [US4] Update the server-concession path in `atlas_play` (T023) to call `render_game_over(winner="user", ...)` using the implemented `render_game_over` from T026 (replace any stub used in T023) — run `pytest tests/contract/ tests/integration/ tests/unit/` and confirm ALL tests PASS
...
- [x] T029 [P] Add `[tool.ruff]` and `[tool.ruff.lint]` sections to `pyproject.toml` (select: `["E", "F", "I", "UP"]`; line-length: 100); add `[tool.mypy]` section (strict = true, python_version = "3.12"); add `[tool.pytest.ini_options]` (asyncio_mode = "auto", testpaths = ["tests"])
- [x] T030 [P] Run `uv run ruff check src/ tests/ scripts/` and fix all reported lint issues across all Python files
- [x] T031 [P] Run `uv run ruff format src/ tests/ scripts/` to auto-format all Python files
- [x] T032 [P] Run `uv run mypy src/` and fix all type errors; ensure all public functions have complete type annotations
- [x] T033 Run full test suite `uv run pytest --tb=short -q` and confirm 100% of tests pass; record final test count in a comment at the bottom of this file
- [x] T034 [P] Update `README.md` with: project description (ATLAS geographic word game, MCP server); installation instructions (`uv sync --all-extras`, `build_corpus.py`); how to connect to Claude Desktop (copy the JSON config from quickstart.md); list of the three tools (`atlas_start`, `atlas_play`, `atlas_concede`) with one-line descriptions
- [x] T035 Run `uv run fastmcp list src/atlas/server.py` and `uv run fastmcp call src/atlas/server.py atlas_start '{}'` to validate the server registers and responds correctly via the FastMCP CLI (quickstart.md validation step)
...
<!-- Final test count: 66 tests (21 unit/game + 18 unit/geography + 9 contract + 18 integration) — all passing (T033) -->
